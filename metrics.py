...
class Sprawl_Metrics(object):
	
	def __init__(self, input_file):
		self.file = input_file

	def agg_developable(self, dataset):
		'''
		Aggregate developable land use. Create new field for aggregate developable. Assumes multiindex (AREA).
		Input: dataset with no duplicate keys (defined as individual cells or municipalities), total area for each land use, and column delinating land developement status
		Return: dataset with an additional column that sums the area of all potentially developable and developed land in cell or muni. 
		'''
	...
	def agg_residential(self, dataset, new_LU_code):
		'''
		Aggregate residential land use. Create new land use code (new_LU_code) for aggregate residential.
		Assumes multiindex (AREA) and uses MASSGIS LUCODES (10:13)
		Input: dataset with no duplicate keys (defined as individual cells or municipalities) and total area for each land use
		Return: dataset with an additional land use column that sums the area of all residential-related land use in a cell or muni.
		'''
		agg_data = dataset.copy()
	...
	def get_cbd_data(self, cbd_file):
		'''
		Formats the data on CDB locations
		Input: csv file with town ID, and longitude and latitude of CBD
		Output: formatted table containing three columns: Municipality, CBD_Lat, CBD_Long
		'''
	...
	def format_gis_data(self):
		'''
		Formats the raw land use data files and group observations by cell and town.
		Input: raw csv file on land use from GIS 
		Output: two datasets on the total area of each land use type - one that aggegates foe each cell and one that aggregates for each town
		'''

		#Import .csv file
		raw_data = pandas.read_csv(self.file)
		landuse_field = gis_field[self.file[-13:-9]]
		...
		agg_grid_data = self.agg_developable(agg_grid_data)
		agg_grid_data = self.agg_residential(agg_grid_data,50)
		...
		agg_muni_data = self.agg_developable(agg_muni_data)
		agg_muni_data = self.agg_residential(agg_muni_data,50)
	...
	def local_proximity(self):
		'''
		Calculates the local proximity metric for each municipality
		Input: raw csv file on land use from GIS
		Output: table with local proximity calculations for each residential, commercial and industrial land use type for each municipality
		'''

		grid, muni = self.format_gis_data()

		print 'Calculating Local Proximity for: ' + str(self.file[-13:-4])
		...
		local_prox_table.to_csv('Local_Prox_'+self.file[-13:-4]+'.csv')

		return local_prox_table



	def global_proximity(self):
		'''
		Calculates the global proximity metric for each municipality
		Input: raw csv file on land use from GIS
		Output: table with global proximity calculations for each residential, commercial and industrial land use type for each municipality
		'''

		grid, muni = self.format_gis_data()

		print 'Calculating Global Proximity for: ' + str(self.file[-13:-4])
		...
		global_prox_table.to_csv('Global_Prox_'+self.file[-13:-4]+'.csv')
		return global_prox_table

	def centrality(self, cbd_file):
		'''
		Calculates the centrality metric for each municipality
		Input: raw csv file on land use from GIS
		Output: table with centrality calculations for each residential, commercial and industrial land use type for each municipality
		'''

		grid, muni = self.format_gis_data()

		cbd = self.get_cbd_data(cbd_file)

		print 'Calculating Centrality for: ' + str(self.file[-13:-4])
		...
		centrality_table.to_csv('Centrality_'+self.file[-13:-4]+'.csv')
		return centrality_table

	def concentration(self):
		'''
		Calculates the concentration metric for each municipality
		Input: raw csv file on land use from GIS
		Output: table with concentration calculations for each residential, commercial and industrial land use type for each municipality
		'''

		grid, muni = self.format_gis_data()

		print 'Calculating Concentration for: ' + str(self.file[-13:-4])
		...
		concentration_table.to_csv('Concentration_'+self.file[-13:-4]+'.csv')
...
class sprawlThread (threading.Thread):
    ...
    def run(self):
        ...
        land_use_analysis = Sprawl_Metrics(self.file)
        land_use_analysis.global_proximity()
        land_use_analysis.local_proximity()
        land_use_analysis.centrality('townhall_geo.csv')
        land_use_analysis.concentration()
        print "Exiting thread for: " + str(self.file[-13:-4])

def main():
	#Global variables: Needed for reading of files. Adjust based on raw data
	global files, fields, landuse_codes, landuse_types, gis_field
